...
3.  Menyediakan bilangan-bilangan `x1`, `y1`, dan `theta` secara acak. Karena ketiga bilangan tersebut mengikuti distribusi seragam, kita menggunakan fungsi `runif()`. Dengan distribusi seragam ini, setiap bilangan dari nilai `min` sampai `max` memiliki kemungkinan yang sama untuk terpilih.
...
Berdasarkan hasil eksperimen jarum Buffon yang terdata dalam `data_eksperimen`, kita dapat melihat estimasi nilai $\pi$-nya pada percobaan yang terakhir, yaitu percobaan ke-1000. Berapa nilai estimasinya? Mari kita menggunakan @lst-estimasi-pi-percobaan-1000 dan lihat luarannya.

```{r}
#| code-fold: show
#| lst-label: lst-estimasi-pi-percobaan-1000
#| lst-cap: 'Menampilkan nilai `estimasi_pi` pada percobaan yang ke-1000'
