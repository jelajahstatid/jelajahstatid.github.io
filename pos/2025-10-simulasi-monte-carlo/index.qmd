...
Koordinat kedua ujung ruas garis tersebut saling berkaitan karena jarak keduanya haruslah $l=1$ satuan. Jika $\theta$ adalah besar sebuah sudut yang dibentuk oleh garis horizontal dan ruas garis tersebut, koordinat $\left( x_{2}, y_{2} \right)$ dapat ditentukan dengan menggunakan @eq-koord-titik-kedua.
...
Berdasarkan @fig-jarum-dan-garis, kita dapat melihat bahwa jarum yang memotong garis pada kertas nilai $h$-nya lebih besar dari jarak vertikal antara titik tengah jarum tersebut dan garis terdekatnya, yaitu $d_{h}$. Padahal, $h=\frac{1}{2}\cdot\sin\left(\theta\right)$. Perhatikan juga bahwa garis-garis pada kertas yang kita buat pada @lst-kertas-bergaris memiliki koordinat-$y$ yang merupakan bilangan bulat. Dengan demikian, kita mendapatkan $d_{h}=\text{min}\left(\left | y_{t}-\left \lfloor y_{t}\right \rfloor\right |,\left | y_{t}-\left \lceil y_{t}\right \rceil\right |\right)$ dengan $y_{t}$ adalah koordinat-$y$ titik tengah jarum tersebut. Dengan demikian, sebuah jarum yang memotong garis memenuhi kondisi berikut.

$$
\frac{1}{2}\sin\left(\theta\right)\geq \text{min}\left(\left | y_{t}-\left \lfloor y_{t}\right \rfloor\right |,\left | y_{t}-\left \lceil y_{t}\right \rceil\right |\right)
$$
...
d_h <- min(                           # <3>
  abs(y_t - floor(y_t)),              # <3>
  abs(y_t - ceiling(y_t))             # <3>
)

potong_garis <- if_else(                # <4>
  h >= d_h, "Memotong", "Tak memotong"  # <4>
)                                       # <4>
...
3.  Menghitung nilai $d_{h}$ dan hasilnya dinamai dengan `d_h`.
4.  Mendefinisikan kondisi yang telah dibahas sebelumnya, dan hasilnya dinamai dengan `potong_garis`.
5.  Mencetak nilai `potong_garis`.
...
data_eksperimen <- data_eksperimen |> 
  mutate(
    ...
    potong = if_else(
      1 / 2 * sin(theta_l) >= pmin(abs(yt - floor(yt)), abs(yt - ceiling(yt))),
      "Memotong",
      "Tak memotong"
    )
  )
