...
Berdasarkan eksperimen kita tersebut, berapakah peluang empiris (frekuensi relatif) jarum yang memotong garis? Kita dapat menjawabnya dengan kode pada @lst-nilai-peluang-empiris.

```{r}
#| code-fold: false
#| lst-label: lst-nilai-peluang-empiris
...
```{r}
#| code-fold: show
#| lst-label: lst-glimpse-data-eksperimen
#| lst-cap: 'Melihat gambaran umum `data_eksperimen`'
glimpse(data_eksperimen)
```
...
Berikutnya, kita akan membuat visualisasi seperti pada @fig-vis-hasil-eksperimen, tetapi dengan menggunakan animasi. Dalam animasi tersebut, kita buat agar jarumnya tampak jatuh ke kertas secara bergiliran. Kita lakukan hal ini dengan kode pada @lst-animasi-jarum-jatuh.
...
Sekarang kita telah memiliki objek `plot_anim_jarum`. Objek tersebut merupakan objek gganim yang menyajikan animasi eksperimen pelemparan jarum ke kertas. Kita cetak objek tersebut, dan hasilnya disajikan pada @fig-animasi-jarum-jatuh.
