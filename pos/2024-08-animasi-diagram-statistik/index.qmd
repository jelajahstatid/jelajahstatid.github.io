...
Penggunaan animasi adalah salah satu solusinya. Pembuatan animasi tersebut dapat dilakukan di [R](https://www.R-project.org/){.external target="_blank"} dengan menggunakan paket [{gganimate}](https://CRAN.R-project.org/package=gganimate){.external target="_blank"}. Tak perlu berlama-lama, mari kita panggil paket tersebut bersamaan dengan paket [{tidyverse}](https://www.tidyverse.org/){.external target="_blank"}. Paket {tidyverse} tersebut berguna untuk mengimpor, mengolah, dan memvisualisasikan data. Sebagai tambahan, sebaiknya dalam R kita juga sudah terinstal paket [{gifski}](%7Bhttps://CRAN.R-project.org/package=gifski%7D){.external target="_blank"}. Paket ini digunakan untuk merender animasi kita nanti menjadi fail gif.
...
Kita impor dan olah data Gapminder seperti pada [pos sebelumnya](/pos/2024-08-statistik-jendela-dunia/index.qmd). Baris kode yang digunakan adalah sebagai berikut.
