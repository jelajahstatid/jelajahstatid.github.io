...
Apa yang kamu lihat dari sebuah foto? Mungkin kamu melihat kenangan yang tersimpan. Mungkin kamu juga ingin menggali pesan yang tersirat. Oke, mari kita melihat sebuah foto atau gambar dari sudut pandang yang berbeda! Kita dapat melihat sebuah gambar sebagai matriks. Perhatikan @fig-meme-epsilon!

![Gambar epsilon yang terlalu kecil](aset/epsilon_terlalu_kecil-01.png){#fig-meme-epsilon}

Mari kita fokus pada gambar kecil dalam meme pada @fig-meme-epsilon. Gambar apakah itu? Ya, itu adalah epsilon. Karena epsilonnya terlalu kecil, mari kita memperbesar gambar tersebut dan menaruhnya ke bidang koordinat Kartesius. Kita lakukan hal itu dengan pemrograman R. Untuk itu, kita panggil paket-paket yang kita perlukan.
...
Paket-paketnya telah siap. Kita lanjut memperbesar gambar epsilon tersebut dan memposisikannya dalam bidang koordinat Kartesius. Perhatikan @fig-matriks-epsilon!

```{r}
#| echo: false
#| label: fig-matriks-epsilon
#| fig-cap: "Gambar epsilon pada bidang koordinat Kartesius"
...
Sekarang, apa yang kamu amati dari @fig-matriks-epsilon? Piksel-piksel dalam gambar tersebut tampak jelas. Setiap piksel berkorespondensi dengan satu warna. Warna tersebut dapat dinyatakan ke dalam tiga warna dasarnya, yaitu RGB (*red*, *green*, dan *blue*). Dengan demikian, kita dapat menyatakan gambar tersebut sebagai tiga matriks warna dasar: $R$, $G$, dan $B$.
...
Nah, sekarang kita dapat melihat sebuah gambar atau foto dengan menggunakan matriks, yaitu tiga matriks warna dasarnya. Dari tiga matriks tersebut, kita dapat mengkonstruksi titik-titik yang merepresentasikan warna tersebut ke dalam ruang dimensi tiga. Kita gunakan perspektif seperti ini untuk memanipulasi sebuah foto sehingga variasi warnanya tereduksi. Tujuan kita berikutnya adalah untuk membuat foto di sebelah kiri pada @fig-foto-asli-dan-hasil-klasterisasi menjadi seperti yang sebelah kanan.

![Foto asli (kiri) dan foto dengan reduksi warna (kanan)](aset/klasterisasi_karuna_donau.png){#fig-foto-asli-dan-hasil-klasterisasi}
...
Tahapan pertamanya adalah mengestrak data warna dari sebuah foto. Foto awal yang kita gunakan adalah foto sebelah kiri pada @fig-foto-asli-dan-hasil-klasterisasi. Untuk melakukannya, kita gunakan kode berikut.
...
Hasil klasterisasi tersebut kita simpan sebagai sebuah objek dengan nama `hasil_krerata`. Terdapat informasi penting yang terkandung dalam `hasil_krerata`, yaitu `cluster` (klaster) dan `centers` (pusat setiap klaster). Kita gunakan kedua informasi tersebut untuk menyederhanakan warna. Kita lakukan pada @sec-menyederhanakan-warna.
...
Sekarang, kita gambarkan kembali piksel-piksel dalam `rgb_foto_klaster_df` pada bidang koordinat Kartesius. Hasilnya ditunjukkan pada @fig-foto-warna-klaster.
...
#| fig-cap: "Foto asli dan hasil reduksi variasi warna foto dengan klasterisasi k-rerata dengan k = 16, 4, dan 2"
#| echo: false
...
#| echo: false
#| fig-cap: "Komposisi warna pada foto-foto yang warnanya tereduksi menjadi k = 2, 4, dan 16"
#| fig-asp: 0.75
...
pr16 <- ringkas_16 |> 
  ...
  labs(
    title = "Klasterisasi dengan k = 16"
  )

plot_grid(
  plot_grid(pr2, pr4, ncol = 2), # first row: two plots
  pr16,                          # second row: one plot
  ncol = 1                       # stack rows
)
